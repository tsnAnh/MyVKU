...
interface ThreadRepo {
    ...
    suspend fun createThread(idToken: String, container: CreateThreadContainer): ForumThread
}

class ThreadRepoImpl : ThreadRepo {
    override suspend fun getThreads(forumId: String) = VKUServiceApi.network.getThreads(forumId)
    override suspend fun createThread(idToken: String, container: CreateThreadContainer) =
        VKUServiceApi.network.createThread(idToken, container)
}