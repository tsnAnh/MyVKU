...
class RetrieveForumsUseCaseImpl : RetrieveForumsUseCase {
    ...
    override fun execute() = liveData(Dispatchers.IO) {
        emit(Resource.Loading<List<NetworkCustomForum>>())
        try {
            emit(Resource.Success<List<NetworkCustomForum>>(forumRepo.getForums()))
        } catch (e: SocketTimeoutException) {
            emit(Resource.Error("Connection Timed Out"))
        } catch (e2: HttpException) {
            emit(Resource.Error("Cannot connect to server!"))
        } catch (t: Throwable) {
